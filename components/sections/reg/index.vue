        ...
        <div class="auth-form__fields">
          <div class="auth-form__roles">
            <div class="role-ratio">
              <span> Заказчик</span>
              <input type="checkbox" :value="'seller'" :disabled="sellerIsChecked"
                @click="handleCheckboxChange('seller')" />
            </div>
            <div class="role-ratio">
              <span>Фрилансер</span>
              <input type="checkbox" :value="'freelancer'" :disabled="freelancerIsChecked"
                @click="handleCheckboxChange('freelancer')" />
            </div>
          </div>
          <fieldset class="fg _small">
            <label>Логин</label>
            <Field v-model="regDetails.username" rules="required|alpha_dash" name="username" type="text"
              placeholder="freelancer777" />
            <ErrorMessage name="username" class="error-message" />
          </fieldset>
          <fieldset class="fg _small">
            <label>Имя</label>
            <Field v-model="regDetails.name" rules="required|alpha" name="name" type="text" placeholder="Александр" />
            <ErrorMessage name="name" class="error-message" />
          </fieldset>
          <fieldset class="fg _small">
            <label>Фамилия</label>
            <Field v-model="regDetails.surname" rules="required|alpha" name="surname" type="text"
              placeholder="Поташов" />
            <ErrorMessage name="surname" class="error-message" />
          </fieldset>
          <fieldset class="fg _small">
            <label>Email адрес</label>
            <Field v-model="regDetails.email" rules="required|email" name="email" type="email"
              placeholder="hello@mail.com" />
            <ErrorMessage name="email" class="error-message" />
          </fieldset>
          <fieldset class="fg _small">
            <label>Пароль</label>
            <Field v-model="regDetails.password" rules="required|min:8" name="password" type="password"
              placeholder="•••••••" />
            <ErrorMessage name="password" class="error-message" />
          </fieldset>
          <fieldset class="fg _small">
            <label>Повторите пароль</label>
            <Field v-model="regDetails.repeatPassword" rules="required|confirmed:@password" name="repeatPassword"
              type="password" placeholder="•••••••" />
            <ErrorMessage name="repeatPassword" class="error-message" />
          </fieldset>
          <UIVButton :disabled="!meta.valid" color="blue" :is-shadow="true" type="submit">
            Создать аккаунт
          </UIVButton>
        </div>
