<template>
  <div class="admin-top">
    <div class="text32 medium-text">Финансы</div>
  </div>

  <div v-if="user?.active_role === 'seller'" class="finance-stat grid-col4">
    <div class="stat-card3">
      <div class="stat-card3__content">
        <div class="stat-card3__title">Заработано</div>
        <div class="stat-card3__num">11.928</div>
        <div class="stat-card3__desc"><span>₽990</span> поступило</div>
      </div>
      <div class="stat-card3__img">
        <img src="/img/stat-icon6.svg" alt="" />
      </div>
    </div>

    <div class="stat-card3">
      <div class="stat-card3__content">
        <div class="stat-card3__title">Выплачено</div>
        <div class="stat-card3__num">912</div>
        <div class="stat-card3__desc"><span>80+</span> новых</div>
      </div>
      <div class="stat-card3__img">
        <img src="/img/stat-icon7.svg" alt="" />
      </div>
    </div>

    <div class="stat-card3">
      <div class="stat-card3__content">
        <div class="stat-card3__title">Ожидание</div>
        <div class="stat-card3__num">820</div>
        <div class="stat-card3__desc"><span>35+</span> новых</div>
      </div>
      <div class="stat-card3__img">
        <img src="/img/stat-icon8.svg" alt="" />
      </div>
    </div>

    <div class="stat-card3">
      <div class="stat-card3__content">
        <div class="stat-card3__title">Доступно для вывода</div>
        <div class="stat-card3__num">8.000</div>
        <div class="stat-card3__desc"><span>290+</span> новых</div>
      </div>
      <div class="stat-card3__img">
        <img src="/img/stat-icon9.svg" alt="" />
      </div>
    </div>
  </div>

  <div class="finance-table m-table">
    <table>
      <thead>
        <tr>
          <th><span>Дата</span></th>
          <th><span>Тип операции</span></th>
          <th><span>Описание</span></th>
          <th><span>Стоимость</span></th>
          <th><span>Сумма</span></th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="i in 8" :key="i" class="finance-row">
          <td>
            <div class="finance-row__title">Дата</div>
            <div class="finance-row__desc light-text">April 9, 2022</div>
          </td>
          <td>
            <div class="finance-row__title">Тип операции</div>
            <div class="finance-row__type" :class="{ _type2: i % 2 }">
              <span>{{ i % 2 ? 'Пополнение' : 'Покупка' }}</span>
            </div>
          </td>
          <td>
            <div class="finance-row__title">Описание</div>
            <div class="finance-row__desc light-text">
              Я разработаю пользовательский интерфейс веб-сайта в Adobe XD или
              Figma.
            </div>
          </td>
          <td>
            <div class="finance-row__title">Стоимость</div>
            <div class="finance-row__desc">₽829</div>
          </td>
          <td>
            <div class="finance-row__title">Сумма</div>
            <div class="finance-row__desc">₽829</div>
          </td>
        </tr>
      </tbody>
    </table>

    <div class="pagination">
      <a href="" class="pagination__btn _prev">
        <svg
          width="15"
          height="15"
          viewBox="0 0 15 15"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M3.8186 8.00232L10.1784 14.3445C10.3868 14.5522 10.7241 14.5518 10.9321 14.3434C11.14 14.1351 11.1395 13.7975 10.9311 13.5897L4.94983 7.62498L10.9313 1.6603C11.1396 1.45244 11.1402 1.11511 10.9324 0.906712C10.8281 0.802244 10.6915 0.75001 10.5548 0.75001C10.4186 0.75001 10.2825 0.801895 10.1784 0.905638L3.8186 7.24766C3.71824 7.34751 3.66193 7.4834 3.66193 7.62498C3.66193 7.76656 3.71841 7.90229 3.8186 8.00232Z"
            fill="#222222"
          />
        </svg>
      </a>

      <a href="" class="pagination__item">1</a>
      <a href="" class="pagination__item _active">2</a>
      <a href="" class="pagination__item">3</a>
      <a href="" class="pagination__item _hide-mob">4</a>
      <a href="" class="pagination__item _hide-tablet">5</a>
      <a href="" class="pagination__item">...</a>
      <a href="" class="pagination__item">20</a>

      <a href="" class="pagination__btn _next">
        <svg
          width="15"
          height="15"
          viewBox="0 0 15 15"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M11.1813 7.24768L4.82149 0.90549C4.61312 0.697817 4.27576 0.698166 4.06774 0.906564C3.85988 1.11493 3.86042 1.45248 4.06882 1.66031L10.05 7.62502L4.0686 13.5897C3.86023 13.7976 3.85969 14.1349 4.06753 14.3433C4.17181 14.4478 4.30842 14.5 4.44503 14.5C4.5813 14.5 4.71737 14.4481 4.82147 14.3444L11.1813 8.00234C11.2816 7.90249 11.338 7.7666 11.338 7.62502C11.338 7.48344 11.2815 7.34771 11.1813 7.24768Z"
            fill="#222222"
          />
        </svg>
      </a>
    </div>

    <div class="pagination__text">
      <div class="text14">1 – 20 of 300+ публикаций доступно</div>
    </div>
  </div>
</template>

<script setup lang="ts">
const userStore = useUserStore();
const { user } = storeToRefs(userStore);
definePageMeta({
  layout: 'profile',
});
</script>

<style scoped></style>
