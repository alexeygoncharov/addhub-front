  ...
  <div class="payments projects-table m-table _tabs-parent _mob-select">
    <div class="tabs-select">
      ...
      <div class="tabs-select__hidden">
        <div class="tabs">
          <div v-for="el in titles" :key="el.value" class="m-tab _tab" :class="{ _active: activeTab === el.value }"
            @click="async () => {
            activeTab = el.value;
            await nextTick();
            updateItems();
        ...
        <div class="modal-body">
          <div class="chat-nav _flex">
            <div class="chat-nav__field">
              <textarea v-model="dispute.message" placeholder="Напишите сообщение"></textarea>
            </div>
            <div class="chat-nav__action _flex">
              <!--v-if-->
              <form class="chat-nav__file">
                <div v-if="files">{{ files.item(0)?.name }}</div>
                <button type="button" class="chat-nav__file-btn" @click="open()">
          ...
          <tr v-for="item of items" :key="item._id" class="reply-row">
            ...
            ">
              <div class="reply-row__action">
                <div v-if="user?.active_role === 'seller' && item.status === 'created'
            " class="reply-row__btn" @click="changeStatus('processing', item._id)">
                  <button class="m-btn m-btn-blue3">
                    <svg fill="#9DA1C6" width="800px" height="800px" viewBox="0 0 1920 1920"
                ...
                <div v-if="user?.active_role === 'seller' &&
            (item.status === 'processing' || item.status === 'refunded')
            " class="reply-row__btn" @click="changeStatus('completed', item._id)">
                  <button class="m-btn m-btn-blue3">
                    <svg width="800px" height="800px" viewBox="0 0 15 15" fill="none"
                      xmlns="http://www.w3.org/2000/svg">
                ...
                <div v-if="item.status === 'completed' && user?.active_role === 'buyer'
            " class="reply-row__btn" @click="changeStatus('paid', item._id)">
                  <button class="m-btn m-btn-blue3">
                    <svg width="800px" height="800px" viewBox="0 0 15 15" fill="none"
                      xmlns="http://www.w3.org/2000/svg">
                      <path fill-rule="evenodd" clip-rule="evenodd"
                ...
                <div v-if="item.status === 'completed' && user?.active_role === 'buyer'
            " class="reply-row__btn" @click="changeStatus('refunded', item._id)">
                  <button class="m-btn m-btn-blue3">
                ...
                <div v-if="item.status === 'completed' || item.status === 'refunded'
            " class="reply-row__btn" @click="reveal(item._id)">
                  <button class="m-btn m-btn-blue3">
                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px"
                      fill="#9DA1C6">
                      <path
                ...
                <div v-if="item.status === 'created'" class="reply-row__btn"
                  @click="changeStatus('canceled', item._id)">
                  <button class="m-btn m-btn-blue3">
    ...
    <UIVPagination v-model="currentPage" :items-per-page="8" :total-items="total" :total-pages="Math.ceil(total / 8)" />
  </div>
</template>
  ...
  if (files.value?.length) {
    ...
    commonStore.uploadFile(formData).then((file) => {
      if (file) {
        ...
        disputeStore.createDispute(dispute.value).then(() => changeStatus('dispute', dispute.value.order));
