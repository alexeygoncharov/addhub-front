<template>
  ...
  <div v-if="isRevealed">
    <OnClickOutside class="modal-screen" @trigger="cancel()">
      <div class="el-dialog">
        <h1 class="el-dialog__header">Обработка запроса</h1>
        <div class="el-dialog__body">
          <div class="title">Дата и время: Апрель 9, 2022</div>
          <div class="title">Пользователь: User12345</div>
          <div class="title">Сумма: ₽1.800</div>
          <br />
          ...
          <div class="dialog-footer">
            <div>
              <button class="m-btn m-btn-gray" @click="cancel()">
                <span>Отмена</span>
              </button>
            </div>
            <div>
              <button class="m-btn m-btn-blue"><span>Утвердить</span></button>
            </div>
            <div>
              <button class="m-btn m-btn-red"><span>Отклонить</span></button>
            </div>
          </div>
        </div>
      </div>
  ...
  <div class="payments m-table">
    <table>
      ...
      <tbody>
        <tr v-for="i in 1" :key="i" class="pay-row">
          <td>
            <div class="pay-row__title">Дата и время</div>
            <div class="pay-row__desc">Апрель 9, 2022</div>
          </td>
          <td>
            <div class="pay-row__title">Пользователь</div>
            <div class="pay-row__desc">User12348</div>
          </td>
          <td>
          ...
          <td>
            <div class="reply-row__action">
              <button class="reply-row__btn" @click="reveal">
                <a class="m-btn m-btn-blue3"
                  ><svg
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
